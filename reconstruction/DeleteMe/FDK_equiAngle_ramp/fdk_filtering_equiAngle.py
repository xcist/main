...
# pre-weighting ramp filter

for Yindex in range(YL):
    for zindex in range(ZL):
        Dgy[Yindex,zindex,:]=(DistD/np.sqrt(DistD**2 + ((zindex-ZCtr)*DeltaZ)**2))*ProjData[Yindex,zindex,:]*math.cos((Yindex-YCtr)*DeltaUW)
