...
extern void fbp(TestStruct *t) {
	...
	float centerX,centerY,centerZ,startangle;
	...
	for(loop=0;loop<RecSize;loop++)
		xCor[loop] = (loop-centerX)*DeltaR+XOffSet;
	for(loop=0;loop<RecSize;loop++)
		yCor[loop] = (loop-centerY)*DeltaR+YOffSet;
    for(loop=0;loop<RecSize;loop++)
        zCor[loop] = (loop-centerZ)*sliceThickness+ZOffSet;
	...
	for (i=0;i<FOILength;i++)
		{
			...
			for (j=0;j<FOIWidth;j++)
			{
				...
				if( (xCor[i]*xCor[i] + yCor[j]*yCor[j])< 2*RadiusSquare)
					...
					t->RecIm[i][j][k] = -t->RecIm[i][j][k]/(2*ProjNum);
				}
            }//for (j=0;j<RecSize;j++)
        }//for (i=0;i<RecSize;i++)
    }//for (k=0;i<RecSize;k++)
