...
//  Launch a photon
//  If it interacts, compute the probability that the scatter angle and
//   absorption are such that the photon makes it to each detector.
#include <iostream>
#include <math.h>
...
float GetTransmissionProbability(Vec3 start, Vec3 stop, int Energy) {
  ...
  return exp(-MuAbsorption[Energy]/10.0*pathLength);
}

void Photon::DoCompton() {
  // We calculate for each detector, the probability of arrival, given that a compton
  // event occurred.  
  if (EnableAccel) {
