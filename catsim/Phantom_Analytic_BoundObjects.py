...
def Phantom_Analytic_BoundObjects(phantObject=None,params=None, debug=False):
    ...
    for i in range(numObjects):
        ...
        if typ == 1:
            ...
            Vx = np.matmul(phantObject[i]['R'], np.matmul(tmp_mat,Vx)) + pars[:3, None]*np.ones((1,Verts))
...
if __name__ == '__main__':
    ...
    #breakpoint()
    Phantom_Analytic_BoundObjects(phantObject=python_in, params=params_in, debug=True)
