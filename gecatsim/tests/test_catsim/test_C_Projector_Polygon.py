...
from gecatsim.pyfiles.C_Projector_Polygon import C_Projector_Polygon


class TestC_Projector_Polygon(unittest.TestCase):

    def test_c_projector_analytic_calls_clib_polygon_projector(self):
        ...
        cfg.clib.polygon_projector = MagicMock()
        cfg.clib.polygon_projector_one_thread = MagicMock()

        C_Projector_Polygon(cfg, 0, 0)

        if os.name == "nt":
            assert cfg.clib.polygon_projector.call_count == 0
            assert cfg.clib.polygon_projector_one_thread.call_count == 1
        else:
            assert cfg.clib.polygon_projector.call_count == 1
            assert cfg.clib.polygon_projector_one_thread.call_count == 0
