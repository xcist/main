...
void DD2FanProjInner(int direction,
		     int nrcols,
		     float *detDistCopy,
		     float previousDist,
		     float *projCopy,
		    ...
		    float* transposeImgPtr)
{
  ...
  if (*(detDist+2) > *(detDist+1))
      ...
      for (rownr=0 ; rownr <= (nrrows-1) ; rownr++)
	{
	   ...
	   * For all distances
	   */
	  DD2FanProjInner(1, nrcols, detDistCopy, previousDist,
  ...
  for (detnr=0 ; detnr <= nrdet-1 ; detnr++)
	...
	} // divide by cos(alpha) and divide by stepsize
      else
	{
	  *newprojCopy++ = 0.;
	  projection++;
...
/*
 * Distance-driven 2D Fan-beam projector
 */
#ifdef WIN32
__declspec(dllexport)
#endif
void DD2FanProj(int nrdet,
		float x0,
		float y0,
		float *xds,
		float *yds,
  ...
  for (viewnr = 0 ; viewnr <= (nrviews-1) ; viewnr++)
      ...
      for (distnr=0 ; distnr<=nrdet ; distnr++)
	{
	  ...
	  *ydiRotCopy++ = (*ydiCopy++ - yCor)*cosAngle
       ...
       * View backprojection
       */
      DD2FanProjView(nrdet, vertical, x0Rot, y0Rot, xdiRot, ydiRot,
