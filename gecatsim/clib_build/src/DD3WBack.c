 ...
 * DD3 boundaries
 */
static void DD3Boundaries(int nrBoundaries,
		   float *pCenters,
		   float *pBoundaries)
{
  int i;
 ...
 * DD3 add transpose
 */
static void DD3AddTranspose(int nrcols,
			    int nrrows,
			    int nrplanes,
			    float *pOrig,
			    float *pTrans)
 ...
 */
static void DD3WBackRow(float imgX,
			float imgXstep,
			int nrcols,
			float imgZstart,
			float imgZstep,
  ...
  while (colnr <= (nrcols-1))
      ...
      while (planenr <= (nrplanes-1))
	{
	  if (imgZ <= nextdetZ) /* next Z boundary is a pixel boundary */
	    {
	      *pImg += dx*(imgZ-previousZ)*(*viewCopy);
	      pImg++;
	      planenr++;
	      previousZ=imgZ;
	      imgZ+=imgZstep;
	    ...
	    {
	      *pImg += dx*(nextdetZ-previousZ)*(*viewCopy);
	      detZcopy++;
	      viewCopy++;
	      previousZ=nextdetZ;
	      nextdetZ=z0 + scale * *detZcopy;
 ...
 */
static void DD3WBackView(float x0,
			 float y0,
			 float z0,
			 int nrdetcols,
			 int nrdetrows,
...

{
  ...
  float dummyfloat, imgX, imgXstep, imgZ, imgZstep, l2;
  ...
  *detXcopy++=1.e12; /* sentinel */
  if (vertical) /* vertical backprojection */
    {
  ...
  else /* horizontal backprojection */
    {
      for (xdistnr=0 ; xdistnr <= (nrxdist-1) ; xdistnr++)
	{
  ...
  *scales=*(scales+1);
  ...
  /*
   * Copy and scale sinogram into view
   */
  viewCopy = view+1+(nrdetrows+2); // skip hor. and vert. margin
  detXcopy=detX+1;
  scalesCopy=scales+1;
	  ...
	  if (*sinogram != 0)
	    {
	      ...
	      detZstep=fabs(*(detZcopy+1)- *detZcopy)* *scalesCopy;
              l2=y0*y0+deltaX*deltaX+dzdx*dzdx*deltaZ*deltaZ;
	      *viewCopy++ = 1/l2*(*sinogram++);
	    }
	  else
	    {
	      *viewCopy++ = 0.;
  ...
  for (rownr=0 ; rownr <= (nrrows-1) ; rownr++)
    {
       ...
       * Call DD3WBackRow
       */
      DD3WBackRow(imgX, imgXstep, nrcols,
...

{
  ...
  /*
   * Allocate memory for transpose image
   */
  pTrans=(float*)calloc(nrcols*nrrows*nrplanes,sizeof(float));
  ...
  for (viewnr = 0 ; viewnr <= (nrviews-1) ; viewnr++)
      ...
      for (zdistnr=0 ; zdistnr<=(nrzdist-1) ; zdistnr++)
	{
      ...
      *(detZshift+nrzdist+1) = 1.5e12; /* sentinel in shifted array */
       ...
       * View backprojection
       */
      DD3WBackView(x0Rot, y0Rot, z0Shift, nrdetcols, nrdetrows,
   ...
   * View backprojection
   */
  DD3AddTranspose(nrrows, nrcols, nrplanes, pTrans, pOrig);  
