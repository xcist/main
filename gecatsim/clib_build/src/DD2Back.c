...
void DD2FanBackInner(int direction,
		     int nrcols,
		     float *detDistCopy,
		     float previousDist,
		     float *projCopy,
...

{
  ...
  while (colnr <= (nrcols-1))
	...
	{
	  *imgCopy += (pixDist-previousDist)*(*projCopy);
	  imgCopy++;
	...
	{
	  *imgCopy += (*detDistCopy-previousDist)*(*projCopy);
...
/*
 * Distance-driven 2D Fan-beam view backprojector
 */
void DD2FanBackView(int nrdet,
		    int vertical,
		    float x0,
		    float y0,
		    float *xdis,
		    ...
		    float* transposeImgPtr)
{
  ...
  for (detnr=0 ; detnr <= nrdet-1 ; detnr++)
    {
      if (*projection != 0)
	{
	  previousDist=(*detDistCopy + *(detDistCopy+1))/2.; //mid det x-coord
	  *newprojCopy = *projection * 
	    sqrt(y0*y0+(previousDist-x0)*(previousDist-x0))
	    // * source-to-xinterface-distance
	    / (fabs(y0)*fabs(*(detDistCopy+1) - *detDistCopy));
	    // / (source-to-xaxis * delta_xinterface)
	} //projections are divided by stepsize and by cos(angle)
      else
	{
	  *newprojCopy = 0.;
	}
  ...
  if (*(detDist+2) > *(detDist+1))
      ...
      for (rownr=0 ; rownr <= (nrrows-1) ; rownr++)
	{
	   ...
	   * For all distances
	   */
	  DD2FanBackInner(1, nrcols, detDistCopy, previousDist,
...
/*
 * Distance-driven 2D Fan-beam backprojector
 */
#ifdef WIN32
__declspec(dllexport)
#endif
void DD2FanBack(int nrdet,
		float x0,
		float y0,
		float *xds,
		float *yds,
  ...
  for (viewnr = 0 ; viewnr <= (nrviews-1) ; viewnr++)
      ...
      for (distnr=0 ; distnr<=nrdet ; distnr++)
	{
	  ...
	  *ydiRotCopy++ = (*ydiCopy++ - yCor)*cosAngle
       ...
       * View backprojection
       */
      DD2FanBackView(nrdet, vertical, x0Rot, y0Rot, xdiRot, ydiRot,
