...
class XcistReconFrame:
    def __init__(self, window):
        self.frame = XcistFrame(window, "RECONSTRUCTION", 500, 250)
        self.frame.frame.grid(column=2, row=2, padx=5, pady=5)
        ...
        self.kernel_lbl = XcistLabel(self.contents, text="Kernel")
        self.kernel_field = Entry(self.contents, width=12)
        self.kernel_lbl.grid(sticky='W', column=0, row=4, pady=4)
        self.kernel_field.grid(sticky='W', column=1, columnspan=2, row=4, padx=(10,5))

        self.btn = Button(self.contents, width=10, text="RECON", bg='red', fg=FG_PRIMARY, font=('Arial 12 bold'))
        self.btn.grid(sticky='SE', column=2, columnspan=3, row=4, rowspan=2, padx=(20,0), pady=(20,0))

        self.image = XcistImage(self.frame.frame, "IMAGE")
